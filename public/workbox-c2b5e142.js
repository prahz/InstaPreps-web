define('./workbox-c2b5e142.js', ['exports'], function (t) {
  ...
  const l = (t) => [u.prefix, t, u.suffix].filter((t) => t && t.length > 0).join('-');
  const f = (t) => t || l(u.precache);
  const w = (t) => t || l(u.runtime);
  function p(t) {
    t.then(() => {});
  }
  const d = new Set();
  class y {
      ...
      if (!this.o)
        return (
          (this.o = await new Promise((t, e) => {
            ...
            (n.onerror = () => e(n.error)),
              (n.onupgradeneeded = (t) => {
                s ? (n.transaction.abort(), n.result.close()) : typeof this.l === 'function' && this.l(t);
              }),
              (n.onsuccess = () => {
                const e = n.result;
    ...
    async getAllMatching(t, { index: e, query: s = null, direction: n = 'next', count: i, includeKeys: r = !1 } = {}) {
      return await this.transaction([t], 'readonly', (a, c) => {
        ...
        const h = e ? o.index(e) : o;
        const u = [];
        const l = h.openCursor(s, n);
        l.onsuccess = () => {
          const t = l.result;
  ...
  class v {
    ...
    async expireEntries() {
      ...
      const e = await this._.expireEntries(t, this.L);
      const s = await self.caches.open(this.m);
  ...
  const U = async ({ request: t, mode: e, plugins: s = [] }) => {
    const n = R(s, 'cacheKeyWillBeUsed');
    let i = t;
    for (const t of n)
      (i = await t.cacheKeyWillBeUsed.call(t, { mode: e, request: i })), typeof i === 'string' && (i = new Request(i));
    return i;
  };
  const L = async ({ cacheName: t, request: e, event: s, matchOptions: n, plugins: i = [] }) => {
    const r = await self.caches.open(t);
    const a = await U({ plugins: i, request: e, mode: 'read' });
  ...
  const x = async ({ cacheName: t, request: e, response: n, event: i, plugins: r = [], matchOptions: c }) => {
    ...
    const l = R(r, 'cacheDidUpdate');
    const f = l.length > 0 ? await L({ cacheName: t, matchOptions: c, request: o }) : null;
    try {
      await u.put(o, h);
    } catch (t) {
      throw (
  ...
  const N = L;
  const b = async ({ request: t, fetchOptions: e, event: n, plugins: i = [] }) => {
    if ((typeof t === 'string' && (t = new Request(t)), n instanceof FetchEvent && n.preloadResponse)) {
      const t = await n.preloadResponse;
      if (t) return t;
    }
    const r = R(i, 'fetchDidFail');
    const a = r.length > 0 ? t.clone() : null;
    try {
      for (const e of i)
  ...
  const O = { cacheWillUpdate: async ({ response: t }) => (t.status === 200 || t.status === 0 ? t : null) };
  let E;
  async function K(t, e) {
    const s = t.clone();
    const n = { headers: new Headers(s.headers), status: s.status, statusText: s.statusText };
    const i = e ? e(n) : n;
    const r = (function () {
      if (void 0 === E) {
  ...
  class P {
    ...
    addToCacheList(t) {
      const e = [];
      for (const n of t) {
        typeof n === 'string' ? e.push(n) : n && void 0 === n.revision && e.push(n.url);
    ...
    async P({ cacheKey: t, url: e, cacheMode: n, event: i, plugins: r, integrity: a }) {
      ...
      let o;
      let h = await b({ event: i, plugins: r, request: c });
  ...
  (t.CacheFirst = class {
    ...
    async handle({ event: t, request: e }) {
      ...
      let n;
      let i = await N({ cacheName: this.m, request: e, event: t, matchOptions: this.k, plugins: this.T });
      if (!i)
        try {
          i = await this.C(e, t);
        } catch (t) {
    ...
    async C(t, e) {
      ...
      const n = s.clone();
      const i = x({ cacheName: this.m, request: t, response: n, event: e, plugins: this.T });
      if (e)
        try {
          e.waitUntil(i);
        } catch (t) {}
    ...
    (t.NetworkFirst = class {
      ...
      async j({ timeoutId: t, request: e, logs: s, event: n }) {
        ...
        if ((t && clearTimeout(t), i || !r)) r = await this.G({ request: e, event: n });
          ...
          const t = r.clone();
          const s = x({ cacheName: this.m, request: e, response: t, event: n, plugins: this.T });
          if (n)
            try {
              n.waitUntil(s);
            } catch (t) {}
    ...
    (t.StaleWhileRevalidate = class {
      ...
      async handle({ event: t, request: e }) {
        ...
        let i;
        let r = await N({ cacheName: this.m, request: e, event: t, matchOptions: this.k, plugins: this.T });
        if (r) {
          if (t)
            try {
              t.waitUntil(n);
      ...
      async C({ request: t, event: e }) {
        const s = await b({ request: t, event: e, fetchOptions: this.D, plugins: this.T });
        const n = x({ cacheName: this.m, request: t, response: s.clone(), event: e, plugins: this.T });
        if (e)
          try {
            e.waitUntil(n);
          } catch (t) {}
